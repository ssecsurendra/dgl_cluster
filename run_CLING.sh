...
for method in "${methods[@]}"; do
  for fanout in "${fanouts[@]}"; do
    ...
    for batch_size in "${batch_sizes[@]}"; do
      ...
      add_spmm_time=true
      output=$(python3 node_classification1.py --dataset=$1 --batch_size=$batch_size --mode=puregpu --fanout=$fanout,$fanout,$fanout --method=$method --epoch=$2)
      #filename="SPMM_time_surendra/$1_F${fanout}_B${batch_size}_puregpu_$2.txt"
      filename="$DGL_HOME/logs/${method}/$1_F${fanout}_B${batch_size}_E$2.txt"
