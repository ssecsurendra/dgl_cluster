...
void similarity_kernel(
    const float* features, const int* indptr, const int* indices, float* edge_weights, int feature_dim
) {
    ...
    for (int e = edge_start + threadIdx.x; e < edge_end; e += blockDim.x) {
        ...
        //float cosine = (norm_a > 0 && norm_b > 0) ? dot / (sqrtf(norm_a) * sqrtf(norm_b)) : 0.0f;
        float raw_cosine = (norm_a > 0 && norm_b > 0) ? dot / (sqrtf(norm_a) * sqrtf(norm_b)) : 0.0f;
        float cosine = fmaxf(raw_cosine, 0.0f);  // Clamp to minimum 0.0
        ...
        //int count_union = (indptr[src + 1] - i) + (indptr[dst + 1] - j) - count_intersection;
...
if __name__ == "__main__":
    ...
    try:
        edge_weights = cp.zeros(size, dtype=cp.float32)
    ...
    similarity_kernel(
            ...
            (
                unified_feat_cp, row_ptr, col_idx,
                edge_weights, f
            )
        )
    ...
    #         (grid_size,), (block_size,),
    #         (
    #             unified_feat_cp, row_ptr, col_idx, edge_weights, f
    #         )
    #     )
    ...
    print("kernel time ", kernel_end_time - kernel_start_time)
    ...
    ##weight_vector = cp.rint(edge_weights).astype(cp.int64)
    ...
    # np.save(f'{output_dir}/{args.dataset}_weight_vector.txt', cp.asnumpy(weight_vector))
    ...
    print(weight_vector_torch.dtype)
    ...
    # adjwgt = cp.asnumpy(weight_vector).tolist()
        # weight_vector_torch = 100
    # G.idtype == F.int64
    # weight_100 = torch.full((len(col_idx),), 1, dtype=torch.int64)
    # x64 = x.to(torch.int64)  # or x.long()
