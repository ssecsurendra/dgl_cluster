...
#             h_N = g.ndata['h_N']
#             h_total = h_N + self.fc(feat)
#             return h_total + self.bias
#
# class SAGE(nn.Module):
#     # def _init_(self, in_size, hid_size, out_size):
#     def __init__(self, in_size, hid_size, out_size):
#         super().__init__()
#         self.layers = nn.ModuleList()
#         # Three-layer Weighted GraphSAGE-mean
#         self.layers.append(WeightedSAGEConv(in_size, hid_size, "mean"))
#         self.layers.append(WeightedSAGEConv(hid_size, hid_size, "mean"))
#         self.layers.append(WeightedSAGEConv(hid_size, out_size, "mean"))
#         self.dropout = nn.Dropout(0.5)
#         self.hid_size = hid_size
#         self.out_size = out_size
#
    # def forward(self, blocks, x, edge_weights):
    #     h = x
    #     for l, (layer, block) in enumerate(zip(self.layers, blocks)):
    #         e_weights = block.edata['weight']  # Get edge weights for the block
    #         h = layer(block, h, e_weights)
    ...
    #     feat = g.ndata["feat"]
    #     edge_weights = g.edata["weight"]  # Edge weights are expected to be stored in g.edata['weight']
    #     sampler = MultiLayerFullNeighborSampler(1, prefetch_node_feats=["feat"], prefetch_edge_feats=["weight"])
    ...
    #             x = feat[input_nodes]
    #             e_weights = edge_weights[blocks[0].edata[dgl.EID]]  # Get edge weights for the block
    #             h = layer(blocks[0], x, e_weights)
    #             if l != len(self.layers) - 1:
    #                 h = F.relu(h)
    #                 h = self.dropout(h)
...
class SAGE(nn.Module):
    ...
    def forward(self, blocks, x, edge_weights):
        ...
        for l, (layer, block) in enumerate(zip(self.layers, blocks)):
            ...
            #print(block.edata['weight'].dtype)
            h = layer(block, h, edge_weight)
            if l != len(self.layers) - 1:
                h = F.relu(h)
                h = self.dropout(h)
        return h
    ...
    def inference(self, g, device, batch_size):
        """Conduct layer-wise inference to get all the node embeddings."""
        feat = g.ndata["feat"]
        # print("Keys of g: ",g.edata.keys())
        # edge_weights = g.edata["_ID"]  # Edge weights are expected to be stored in g.edata['weight']
        ...
        for l, layer in enumerate(self.layers):
            ...
            for input_nodes, output_nodes, blocks in tqdm.tqdm(dataloader):
                ...
                #h = layer(blocks[0], x, edge_weight)  # len(blocks) = 1
...
def evaluate(model, graph, dataloader, num_classes):
    ...
    for it, (input_nodes, output_nodes, blocks) in enumerate(dataloader):
        with torch.no_grad():
            ...
            y_hats.append(model(blocks, x, blocks[0].edata['weight']))
    return MF.accuracy(
        torch.cat(y_hats),
        torch.cat(ys),
        task="multiclass",
...
def train(args, device, g, 
    ...
    for epoch in range(args.epoch):
        ...
        for it, (input_nodes, output_nodes, blocks) in enumerate(
            ...
            y_hat = model(blocks, x, blocks[0].edata['weight'])
...
if __name__ == "__main__":
    ...
    #cluster_id = cluster_id.to("cuda" if args.mode == "puregpu" else "c pu")
    ...
    #out_size = 107
    # model = WeightedSAGE(in_size, 256, out_size).to(device)
