...
if __name__ == "__main__":
    ...
    columns = ['Data']
    file = pd.read_csv('./output.txt',names=columns)
    Data=file['Data']
    #Data=np.array(Data)
    deg_centrality=np.array(Data)
    print("deg_centrality:",deg_centrality)

    #deg_centrality = dgl.ndarray.array(Data)
    #deg_centrality = deg_centrality.to('cuda')
    ...
    # #deg_centrality = deg_centrality.to('cuda')
    # sorted_col_idx = np.empty_like(col_idx)
    #sorted_col_idx = sorted_col_idx.to('cuda')
    start_time = time.time()

    for u in tqdm(range(len(row_ptr) - 1), desc="processing nodes"):
        start, end = row_ptr[u], row_ptr[u+1]
        neighbors = col_idx[start:end]
        
        # Get the sort order based on descending degree centrality
        order = np.argsort(-deg_centrality[neighbors])
        
        # Apply the order
        sorted_col_idx[start:end] = neighbors[order]
    file_path1 = './sorted_degreea2d.txt'
    with open(file_path1, "w") as file:
        for value in sorted_col_idx:
            file.write(f"{value}\n")
