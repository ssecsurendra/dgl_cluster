...
for fanout in "${fanouts[@]}"; do
  ...
  for batch_size in "${batch_sizes[@]}"; do
    output=$(python3 graphsage_e2e.py --dataset=$1 --batchsize=$batch_size  --samples=$fanout,$fanout,$fanout --num-epoch=$2)
    #filename="SPMM_time_surendra/$1_F${fanout}_B${batch_size}_puregpu_$2.txt"
    filename="time_surendra/$1_F${fanout}_B${batch_size}_$2.txt"
